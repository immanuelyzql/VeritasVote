...
// Alamat Wallet Dari Anvil 0x3Aa5ebB10DC797CAC828524e59A333d0A371443c
...
export default function VeritasVote() {
  ...
  const isAdmin = address?.toLowerCase() === (adminAddr as string)?.toLowerCase()
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans selection:bg-blue-500/30">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-center mb-20">
          <div className="flex-1 text-center lg:text-left">
            ...
            <p className="text-slate-400 text-lg max-w-xl leading-relaxed">
              ...
              Smart Contract Ethereum. Transparan, Akuntabel, dan Terdesentralisasi.
            </p>
          </div>
        ...
        {/* ADMIN CONTROL DASHBOARD (BENTO STYLE) */}
        {isAdmin && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
            <div className="md:col-span-2 bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden">
              ...
              {!isEnded ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <input placeholder="Nama Lengkap" className="bg-slate-800/50 border border-slate-700 p-4 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 transition-all" value={form.nama} onChange={e => setForm({...form, nama: e.target.value})} />
                  <input placeholder="Wallet Address (0x...)" className="bg-slate-800/50 border border-slate-700 p-4 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 transition-all font-mono text-sm" value={form.wallet} onChange={e => setForm({...form, wallet: e.target.value})} />
                ...
                <div className="bg-blue-500/10 border border-blue-500/20 p-6 rounded-2xl text-center">
                  <p className="text-blue-400 font-bold">Pendaftaran sudah ditutup karena sesi voting telah berakhir.</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col justify-between">
              <div>
                <h3 className="text-xl font-black text-white mb-2">FINALISASI</h3>
                <p className="text-slate-500 text-sm">Tutup sesi dan kirim reward 2 ETH ke pemenang suara terbanyak.</p>
              </div>
              
              {!isEnded ? (
                <button 
                  onClick={() => {
                    if (!confirmFinish) { setConfirmFinish(true); setTimeout(() => setConfirmFinish(false), 3000); }
                    else { writeContract({ abi: ABI, address: CONTRACT_ADDRESS, functionName: 'selesaikanPemilihan' }); setConfirmFinish(false); }
                  }} 
                  className={`w-full py-5 rounded-2xl font-black flex items-center justify-center gap-3 transition-all duration-300 ${confirmFinish ? 'bg-orange-600 scale-105' : 'bg-slate-800 hover:bg-slate-700'} text-white shadow-xl`}
                >
                  {confirmFinish ? 'KONFIRMASI AKHIR' : <><Trophy size={20} className="text-yellow-500" /> Tutup Sesi</>}
                </button>
              ) : (
                <button 
                  onClick={() => writeContract({ abi: ABI, address: CONTRACT_ADDRESS, functionName: 'resetVoting' })}
                  className="w-full bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white py-5 rounded-2xl font-black flex items-center justify-center gap-3 transition-all border border-red-500/20"
                >
        ...
        <div className="mb-8 flex items-center justify-between">
          <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Daftar Kandidat Terpilih</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {kandidatList?.map((k) => (
            <div key={k.id.toString()} className="bg-slate-900 border border-slate-800 rounded-4xl p-8 hover:border-blue-500/50 transition-all duration-500 group relative">
              ...
              <h4 className="text-center font-black text-2xl text-white mb-2 tracking-tight uppercase">{k.nama}</h4>
